...
try:
    ...
    from interface import (
        ...
        IMAGEDATA_FROG, IMAGEDATA_MUD, IMAGEDATA_WATER,
        IMAGEDATA_OIL, IMAGEDATA_FIRE
    )
except ImportError:
    IMAGEDATA_PLAYER = IMAGEDATA_TARGET = IMAGEDATA_PLANT = IMAGEDATA_FROG = ""
    IMAGEDATA_MUD = IMAGEDATA_WATER = IMAGEDATA_OIL = IMAGEDATA_FIRE = ""
...
class ReplayInterface(tk.Frame):
    def __init__(self, target_path, mongo_uri="mongodb://localhost:27017/", db_name="gardener_db"):
        ...
        try:
            ...
            self.mud_img = tk.PhotoImage(data=IMAGEDATA_MUD)
            self.water_img = tk.PhotoImage(data=IMAGEDATA_WATER)
            self.oil_img = tk.PhotoImage(data=IMAGEDATA_OIL)
            self.fire_img = tk.PhotoImage(data=IMAGEDATA_FIRE)
        except:
            self.player_img = self.target_img = self.plant_img = self.frog_img = None
            self.mud_img = self.water_img = self.oil_img = self.fire_img = None
        ...
        # Legend Frame in Replay
        self.legend_frame = tk.Frame(canvas_frame, width=150, background="white", borderwidth=1, relief="sunken")
        ...
        add_legend_item(self.player_img, "Player")
        add_legend_item(self.target_img, "Target")
        add_legend_item(self.plant_img, "Plant")
        add_legend_item(self.frog_img, "Frog")
        add_legend_item(self.mud_img, "Mud")
        add_legend_item(self.water_img, "Water")
        add_legend_item(self.oil_img, "Oil")
        add_legend_item(self.fire_img, "Fire")
    ...
    def load_from_json(self, path):
        with open(path, 'r') as f:
            ...
            # Load new properties
            self.properties = {
                "mud": [tuple(p) for p in data["env"].get("mud", [])],
                "water": [tuple(p) for p in data["env"].get("water", [])],
                "oil": [tuple(p) for p in data["env"].get("oil", [])],
                "fire": [tuple(p) for p in data["env"].get("fire", [])],
            }
            # Load frogs cache
    ...
    def load_from_mongo(self):
        ...
        for ts, content in answers:
            ...
            if pp and st:
                # Normalize timestamp: Dpsr might truncate :00
                normalized_ts = ts
                if len(ts) == 16:
                    normalized_ts += ":00"
                self.states.append({"timestamp": normalized_ts, "player": (int(pp.group(1)), int(pp.group(2))), "step": int(st.group(1))})
        ...
        self.properties = {
            "mud": [(int(p["C"]), int(p["R"])) for p in first_doc.get("mud", [])],
            "water": [(int(p["C"]), int(p["R"])) for p in first_doc.get("water", [])],
            "oil": [(int(p["C"]), int(p["R"])) for p in first_doc.get("oil", [])],
            "fire": [(int(p["C"]), int(p["R"])) for p in first_doc.get("fire", [])],
        }
    ...
    def draw_static_grid(self):
        ...
        # Add new properties
        for p_type, locations in self.properties.items():
            img = None
            if p_type == "mud": img = self.mud_img
            elif p_type == "water": img = self.water_img
            elif p_type == "oil": img = self.oil_img
            elif p_type == "fire": img = self.fire_img
            
            for i, loc in enumerate(locations):
                self.add_piece(f"prop_{p_type}_{i}", img, loc, "brown")
