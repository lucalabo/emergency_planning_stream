...
def parse_dpsr_log(file_path):
        ...
        for line in f:
            ...
            if content_match:
                ...
                if True:
                    ...
                    # NEW: Parse Fire Penalties
                    for m in fire_penalty_regex.finditer(content):
                        unique_fire_penalties.add(m.group(1)) # T
                        
                    # NEW: Parse Mud Penalties
                    for m in mud_penalty_regex.finditer(content):
                        unique_mud_penalties.add((m.group(1), m.group(2))) # C, R
    ...
    print(f"{metrics['fire_penalties']:03d} fire penalties")
    print(f"{metrics['mud_penalties']:03d} mud penalties (unique cells)")
