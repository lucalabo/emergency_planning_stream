...
class StreamGame:
    ...
    def update_environment(self,cont_t):
        ...
        # Populate new norm cell types
        for loc, props in self.instance.properties.items():
            for p_type in props:
                if p_type in state_data:
                    state_data[p_type].append({"C": str(loc[0]), "R": str(loc[1])})
        
        # Only send player, target, and walls on the first update
        ...
        state_data["wall"] = [{"C": str(w[0]), "R": str(w[1])} for w in self.instance.walls]
